    ...
    public partial class CreatePublication : Form
    {
        ...
        private void CreatePublication_Load(object sender, EventArgs e)
        {
            ...
            if (_data != null)
            {
                ...
                doi.Text = _data.Doi;
                count.Value = _data.Pages;
                date.Value = _data.Dates;
                authorslist.SelectedItems.Clear();

                for (int i = 0; i < publisher.Items.Count; i++)
                {
                    if ((publisher.Items[i] as Publisher).Id == _data.PublicationHouseId)
                    {
        ...
        private void create_Click(object sender, EventArgs e)
        {
            ...
            mainForm.AddNewToDb(new Work()
            {
                Id = _Id,
                Title = title.Text,
                Doi = doi.Text,
                UdcId = Udcs.Single(p => p.Id == (udc.SelectedItem as UDK).Id).Id,
                Pages = Convert.ToInt32(count.Value),
                Dates = date.Value.Date,
                PublicationHouseId = Publishers.Single(p => p.Id == (publisher.SelectedItem as Publisher).Id).Id,
                AuthorsIds = authors
            });
