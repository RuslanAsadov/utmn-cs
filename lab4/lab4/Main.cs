...
namespace lab3
{
    ...
    {
        private string dbPath = @"C:\Users\ruslanasadov\Documents\Projects\lab4\database.accdb";
        ...
        public void AddNewToDb(Work newItem)
        {
            ...
            if (existingIndex != -1)
            {
                command.CommandText = "UPDATE Works SET title = '" + newItem.Title + "', pages = '" + newItem.Pages + "', udcId = '" + newItem.UdcId + "', dates = '" + newItem.Dates.ToString("dd.MM.yyy H:mm:ss") + "', publicationHouseId = '" + newItem.PublicationHouseId + "', doi = '" + newItem.Doi + "' WHERE id = " + newItem.Id;
                publications[existingIndex] = newItem;
                command.ExecuteReader();
            }
            else
            {
                command.CommandText = "INSERT INTO Works (title, pages, udcId, dates, publicationHouseId, doi) VALUES('" + newItem.Title + "', '" + newItem.Pages + "', '" + newItem.UdcId + "', '" + newItem.Dates.ToString("dd.MM.yyy H:mm:ss") + "', '" + newItem.PublicationHouseId + "', '" + newItem.Doi + "'); ";
                ...
                switch (entityName)
                {
                    ...
                    case "Works":
                        ...
                        AddNew(new Work()
                        {
                            ...
                            Pages = Convert.ToInt32(reader.GetValue(2).ToString()),
                            UdcId = reader.GetValue(3).ToString(),
                            Dates = DateTime.Parse(reader.GetValue(4).ToString()),
                            PublicationHouseId = reader.GetValue(5).ToString(),
